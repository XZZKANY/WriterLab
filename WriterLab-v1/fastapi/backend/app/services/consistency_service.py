...
from app.services.ai_gateway_service import GatewayCallResult, call_ai_gateway
        ...
        issues.append(
            {
                ...
                "fix_suggestion": "统一发色描述，避免让读者误以为人物外貌发生了未交代的变化。",
                "evidence_json": {
                    "character": character.name,
                    "appearance": appearance,
...
) -> tuple[list[dict[str, Any]], GatewayCallResult | None]:
    context_bundle = bundle or build_scene_context(scene, db)
    prompt = (
...
def scan_scene_consistency(
    ...
    *,
...
) -> list[ConsistencyIssue] | tuple[list[ConsistencyIssue], GatewayCallResult | None]:
    ...
    llm_issue_specs, llm_gateway_result = _llm_issues(scene, text, db, bundle=bundle, provider_mode=provider_mode, fixture_scenario=fixture_scenario)
    issue_specs.extend(llm_issue_specs)
    ...
    if return_gateway_result:
        return issues, llm_gateway_result
    return issues
