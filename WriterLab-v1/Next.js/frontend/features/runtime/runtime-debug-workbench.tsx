...
export default function RuntimeDebugWorkbench() {
  ...
  return <WorkspaceShell title="运行诊断" eyebrow="WriterLab" description="集中查看 Workflow Debug、Provider Runtime、Smoke Console、Provider Matrix 与 Context Compiler。" actions={<button className={buttonClass("rounded-full border border-white/10 px-4 py-2 text-sm", busyKey === "refreshAll")} onClick={() => void refreshAll()} disabled={busyKey === "refreshAll"} type="button">{busyKey === "refreshAll" ? "刷新中..." : "刷新全部"}</button>}>
    ...
    <section className={section}>
      <div className="flex items-center justify-between gap-4"><div><h2 className="text-xl font-semibold">Provider Runtime</h2><p className="mt-1 text-sm text-zinc-500">API 配置、provider 状态、step readiness 与 profile 明细。</p></div><button className="rounded-full border border-white/10 px-4 py-2 text-sm" onClick={() => void loadProviderRuntimeState().catch((nextError) => setError(toErrorMessage(nextError, "读取 provider runtime 失败")))} type="button">Refresh</button></div>
